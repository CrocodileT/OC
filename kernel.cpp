...
void erase_buf() { //очистка буфера ввода 
	for (int i = 0; i < 41; ++i) {
		buffer[i] = '\0';
	}
	leng_buf = 0;
...
//Функция приводящая время в вид 23:12:12
...
void on_key(unsigned char scan_code)
{
	...
	{
		if (cmp(buffer, "clear", 5) == 1 && leng_buf == 5) {//Очистка
		...
		else if (cmp(buffer, "nsconv", 6) == 1 && leng_buf < 40) {//Конвертор чисел из одной системы исчисления в другую
			str_pos++;
		...
		else if (cmp(buffer, "shutdown", 8) == 1 && leng_buf == 8) {//Выключение
			clear();
		...
		else if (cmp(buffer, "posixtime", 9) == 1 && leng_buf < 40) {//время которое сейчас
			str_pos++;                                               //На вход передается время в секунда прошедшее с 1970 года
			int index = 10;
			int num[30];
			int leng_num = 0;
			long int N = 0;
		...
		else if (cmp(buffer, "wintime", 7) == 1 && leng_buf < 40) {//На вход передается время в секунда прошедшее с 1970 года в милисекундах
		    str_pos++;
			int index = 8;
			int num1[15];
			int num2[15];
...
void keyb_process_keys() {  // Проверка что буфер PS/2 клавиатуры не пуст (младший бит присутствует)  
	if (inb(0x64) & 0x01) {
